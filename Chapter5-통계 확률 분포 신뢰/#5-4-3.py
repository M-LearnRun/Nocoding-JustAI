...
sample_size = 100
z_value = 1.96       # 95% 한쪽 신뢰수준 (Z_0.025)
...
plt.axvline(lower_bound, color='blue', linestyle='-', label='95% 하한 신뢰구간')
 
# 하한 구간은 하늘색으로, 나머지 구간은 빨간색으로
plt.fill_between(x, y, where=(x >= lower_bound), color='skyblue', alpha=0.5, label='95% 신뢰구간 (하한)')
plt.fill_between(x, y, where=(x < lower_bound), color='pink', alpha=0.5, label='하한 바깥 구간')
 
# 그래프 설정
plt.title("한쪽 신뢰구간과 정규분포")
plt.xlabel("주행 거리 (km)")
plt.ylabel("확률 밀도 함수")
plt.legend()
plt.grid(True)
